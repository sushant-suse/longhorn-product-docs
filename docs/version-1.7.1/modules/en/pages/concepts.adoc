...
To install Longhorn in an air gapped environment, refer to xref:deploy/install/airgap.adoc[this section].
...
Besides, the space usage of the cluster worker nodes is collected and illustrated by the Longhorn UI. See xref:nodes-and-volumes/nodes/node-space-usage.adoc[here] for details.
...
For more introductions about the volume-size related concepts, see this xref:nodes-and-volumes/volumes/volume-size.adoc[doc] for more details.
