...
Argo CD currently does not support the PreDelete resource hook. Instead of running `argocd app delete longhorn` directly, you must uninstall {longhorn-product-name} using `kubectl` to prevent dangling resources from remaining in the `longhorn-system` namespace.
