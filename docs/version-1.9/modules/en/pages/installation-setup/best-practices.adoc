...
The following Linux OS distributions and versions have been verified during the v{patch-version} release testing (you can replace v{patch-version} with your desired Longhorn version). However, this does not imply that {longhorn-product-name} exclusively supports these distributions. Essentially, {longhorn-product-name} should function well on any certified Kubernetes cluster running on Linux nodes with a wide range of general-purpose operating systems, as well as verified container-optimized operating systems like SLE Micro.
