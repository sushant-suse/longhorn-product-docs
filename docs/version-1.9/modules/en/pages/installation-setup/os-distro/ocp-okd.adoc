...
{longhorn-product-name} will use the directory `/var/lib/longhorn` as default storage mount point and that means {longhorn-product-name} uses the root device as the default storage. If you do not want to use the root device as the {longhorn-product-name} storage, set *_defaultSettings.createDefaultDiskLabeledNodes_* true when installing {longhorn-product-name} using Helm:
