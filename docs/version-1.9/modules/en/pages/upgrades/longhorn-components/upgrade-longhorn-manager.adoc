...
For Kubernetes clusters managed by Rancher, you can upgrade the {longhorn-product-name} App by updating the repository to reference a newer chart version.  
This method applies to Rancher Prime v2.12.3 and later.

[NOTE]
====
If your existing {longhorn-product-name} App was installed by using the deprecated *Cluster Manager* (old UI), migrate to the chart in the *Cluster Explorer* (new UI) before proceeding.  
For migration steps, see the https://longhorn.io/kb/how-to-migrate-longhorn-chart-installed-in-old-rancher-ui-to-the-chart-in-new-rancher-ui/[migration guide].
====
