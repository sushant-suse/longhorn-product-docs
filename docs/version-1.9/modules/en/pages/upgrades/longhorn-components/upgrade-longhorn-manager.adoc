...
[NOTE]
====
Because the Cluster Manager (old UI) is being deprecated, the instruction to migrate the existing {longhorn-product-name} installation to the Longhorn chart in the Cluster Explorer (new UI) can be found https://longhorn.io/kb/how-to-migrate-longhorn-chart-installed-in-old-rancher-ui-to-the-chart-in-new-rancher-ui/[here].
====
...
helm:
  ...
  chart: longhorn
  version: v{patch-version} # Replace with the Longhorn version you would like to upgrade to
...
spec:
  ...
  sources:
    - chart: longhorn
      ...
      targetRevision: v{patch-version} # Replace with the Longhorn version you would like to upgrade to
----
