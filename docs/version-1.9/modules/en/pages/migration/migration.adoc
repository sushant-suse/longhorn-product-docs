...
+
Or, you can execute this link:{attachmentsdir}/v1.9.1-longhorn-crd.sh[script] to `longhorn-crd` chart.
+
...
After ensuring the `longhorn-crd` Helm release is uninstalled but the CRDs are kept, you need to update the ownership labels on the existing Longhorn CRDs to prepare for the main `longhorn` Helm chart. Apply the following link:{attachmentsdir}/v1.9.1-longhorn-main.sh[script] to perform the replacement.
