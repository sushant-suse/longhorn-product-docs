...
longhorn                                         	longhorn-system    	1       	2025-09-24 02:58:40.877484249 +0000 UTC	deployed	longhorn-106.3.1+up1.9.2                                                                               	v1.9.2
longhorn-crd                                     	longhorn-system    	1       	2025-09-24 02:58:38.192579711 +0000 UTC	deployed	longhorn-crd-106.3.1+up1.9.2                                                                           	v1.9.2
----
...
----
+
In this example, the release version is `106.3.1+up1.9.2`.

. Clone the `rancher/charts` repository containing the specific version of the `longhorn-crd` chart that matches your current release (for example, `106.2.1+up1.9.2` for Longhorn v1.9.2):
+
[,shell]
----
...
+
After running the command `bash ./download-longhorn-crd-chart.sh 106.3.1+up1.9.2`, the `longhorn-crd` chart is downloaded to the local directory `106.3.1+up1.9.2`. Then, open `106.3.1+up1.9.2/Chart.yaml` to verify that the chart version matches `106.3.1+up1.9.2`.
...
[,shell]
----
bash ./patch-resource-policy-annotation.sh 106.3.1+up1.9.2/templates/crds.yaml
----
+
...
[,shell]
----
vimdiff /tmp/crds.yaml.original 106.3.1+up1.9.2/templates/crds.yaml
----
...
[,shell]
----
helm upgrade longhorn-crd -n longhorn-system ./106.3.1+up1.9.2
----
