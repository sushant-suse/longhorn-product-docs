...
* Based on this nominal size, the replicas will be scheduled to those nodes that have enough allocatable space during the volume creation. (See xref:nodes/node-space-usage.adoc[this doc] for more info about node allocation size.)
