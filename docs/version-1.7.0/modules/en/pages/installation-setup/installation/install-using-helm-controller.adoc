...
You can install Longhorn using the Helm chart controller built into RKE2 and K3s.
...
https://github.com/longhorn/longhorn/blob/v1.7.0/scripts/environment_check.sh[This script] can be used to check the Longhorn environment for potential issues.
...
[IMPORTANT]
====
* Ensure that `spec.failurePolicy` is set to "abort".  The only other value is the default: "reinstall", which performs an uninstall of Longhorn.  With "abort", it retries periodically, giving the user a chance to fix the problem.
+
...
[NOTE]
====
Deleting the helmchart CR will initiate an uninstall of Longhorn.
====
