...
The following requirements must be met before installing {longhorn-product-name} on a Talos Linux cluster.
...
The most straightforward method is to patch the extensions onto existing Talos Linux nodes. Alternatively, you can use the https://factory.talos.dev[Talos Linux Image Factory] and select the desired extensions under *System Extensions*. This approach is especially useful when combined with Infrastructure as Code (IaC) tools.
...
Since Talos Linux deprecated `.machine.disks`, it is recommended to use `UserVolumeConfig` to mount a disk for {longhorn-product-name}. See https://www.talos.dev/v1.10/introduction/what-is-new/#user-volumes[What's new in Talos v1.10] for more details.
...
[NOTE]
====
For more disk configuration options, see the https://www.talos.dev/v1.10/talos-guides/configuration/disk-management/#disk-layout[Talos documentation].
====
...
The xref:longhorn-system/settings.adoc#_default_data_path[default data path] for Longhorn is `/var/lib/longhorn`. To use the configuration below in Talos, first set your default data path to `/var/mnt/longhorn`. The method to do this depends on your xref:longhorn-system/customize-default-settings.adoc[deployment method].
...
Next, create a `UserVolumeConfig` to mount the disk for {longhorn-product-name}. It would be automatically mounted to `/var/mnt/longhorn` on the configured node.
...
For detailed instructions on `UserVolumeConfig` and `VolumeConfig`, see the Talos documentation on https://www.talos.dev/v1.10/reference/configuration/block/[Block configuration]. Refer to the https://docs.siderolabs.com/talos/v1.11/reference/api[Talos API Reference] for more configuration details.
...
The `--preserve` flag is no longer required. It is now automatically set for the `talosctl` upgrade command, as noted https://www.talos.dev/v1.8/introduction/what-is-new/#upgrades[here].
