...
| bool
| `false`
| Setting that allows {longhorn-product-name} UI to run on a Rancher Windows cluster.
...
| string
| `""`
| Registry for the CSI attacher image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.attacher.repository`
| string
| `"longhornio/csi-attacher"`
| Repository for the CSI attacher image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.attacher.tag`
| string
| `"v4.10.0-20260428"`
| Tag for the CSI attacher image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.livenessProbe.registry`
| string
| `""`
| Registry for the CSI liveness probe image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.livenessProbe.repository`
| string
| `"longhornio/livenessprobe"`
| Repository for the CSI liveness probe image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.livenessProbe.tag`
| string
| `"v2.17.0-20260428"`
| Tag for the CSI liveness probe image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.nodeDriverRegistrar.registry`
| string
| `""`
| Registry for the CSI Node Driver Registrar image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.nodeDriverRegistrar.repository`
| string
| `"longhornio/csi-node-driver-registrar"`
| Repository for the CSI Node Driver Registrar image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.nodeDriverRegistrar.tag`
| string
| `"v2.15.0-20260428"`
| Tag for the CSI Node Driver Registrar image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.provisioner.registry`
| string
| `""`
| Registry for the CSI Provisioner image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.provisioner.repository`
| string
| `"longhornio/csi-provisioner"`
| Repository for the CSI Provisioner image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.provisioner.tag`
| string
| `"v5.3.0-20260428"`
| Tag for the CSI Provisioner image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.resizer.registry`
| string
| `""`
| Registry for the CSI Resizer image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.resizer.repository`
| string
| `"longhornio/csi-resizer"`
| Repository for the CSI Resizer image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.resizer.tag`
| string
| `"v2.0.0-20260428"`
| Tag for the CSI Resizer image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.snapshotter.registry`
| string
| `""`
| Registry for the CSI Snapshotter image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.snapshotter.repository`
| string
| `"longhornio/csi-snapshotter"`
| Repository for the CSI Snapshotter image. When unspecified, {longhorn-product-name} uses the default value.

| `image.csi.snapshotter.tag`
| string
| `"v8.4.0-20260428"`
| Tag for the CSI Snapshotter image. When unspecified, {longhorn-product-name} uses the default value.

| `image.longhorn.backingImageManager.registry`
| string
| `""`
| Registry for the Backing Image Manager image. When unspecified, {longhorn-product-name} uses the default value.

| `image.longhorn.backingImageManager.repository`
| string
| `"longhornio/backing-image-manager"`
| Repository for the Backing Image Manager image. When unspecified, {longhorn-product-name} uses the default value.

| `image.longhorn.backingImageManager.tag`
| string
| `"v1.11.2"`
| Tag for the Backing Image Manager image. When unspecified, {longhorn-product-name} uses the default value.
...
| string
| `""`
| Registry for the {longhorn-product-name} UI image.

| `image.longhorn.ui.repository`
| string
| `"longhornio/longhorn-ui"`
| Repository for the {longhorn-product-name} UI image.

| `image.longhorn.ui.tag`
| string
| `"v1.11.2"`
| Tag for the {longhorn-product-name} UI image.
...
| NodePort port number for Longhorn Manager. When unspecified, {longhorn-product-name} selects a free port between 30000 and 32767.

| `service.manager.type`
| Service type for Longhorn Manager.

| `service.ui.annotations`
| Annotation for the {longhorn-product-name} UI service.
...
| `service.ui.nodePort`
| NodePort port number for {longhorn-product-name} UI. When unspecified, {longhorn-product-name} selects a free port between 30000 and 32767.

| `service.ui.type`
| Service type for {longhorn-product-name} UI. (Options: "ClusterIP", "NodePort", "LoadBalancer", "Rancher-Proxy")
|===
...
| string
| `nil`
| Data source type of a backing image used in a Longhorn StorageClass. If the backing image exists in the cluster, {longhorn-product-name} uses this setting to verify the image. If the backing image does not exist, {longhorn-product-name} creates one using the specified data source type.
...
| string
| `nil`
| Backing image to be used for creating and restoring volumes in a Longhorn StorageClass. When no backing images are available, specify the data source type and parameters that {longhorn-product-name} can use to create a backing image.
...
| string
| `""`
| Node selector for the default Longhorn StorageClass. {longhorn-product-name} uses only nodes with the specified tags for storing volume data. (Examples: "storage,fast")

| `persistence.disableRevisionCounter`
| string
| `"true"`
| Setting that disables the revision counter and thereby prevents {longhorn-product-name} from tracking all write operations to a volume. When salvaging a volume, {longhorn-product-name} uses properties of the volume-head-xxx.img file (the last file size and the last time the file was modified) to select the replica to be used for volume recovery.
...
| Replica count of the CSI Attacher. When unspecified, {longhorn-product-name} uses the default value (`"3"`).

| `csi.kubeletRootDir`
| kubelet root directory. When unspecified, {longhorn-product-name} uses the default value.

| `csi.podAntiAffinityPreset`
| Configures Pod anti-affinity to prevent multiple instances on the same node. Use soft (tries to separate) or hard (must separate). When unspecified, {longhorn-product-name} uses the default value (`"soft"`).

| `csi.provisionerReplicaCount`
| Replica count of the CSI Provisioner. When unspecified, {longhorn-product-name} uses the default value (`"3"`).

| `csi.resizerReplicaCount`
| Replica count of the CSI Resizer. When unspecified, {longhorn-product-name} uses the default value (`"3"`).

| `csi.snapshotterReplicaCount`
| Replica count of the CSI Snapshotter. When unspecified, {longhorn-product-name} uses the default value (`"3"`).
|===

=== Longhorn Manager Settings

{longhorn-product-name} consists of user-deployed components (for example, Longhorn Manager, Longhorn Driver, and {longhorn-product-name} UI) and system-managed components (for example, Instance Manager, Backing Image Manager, Share Manager, CSI Driver, and Engine Image). The following settings only apply to Longhorn Manager.
...
{longhorn-product-name} consists of user-deployed components (for example, Longhorn Manager, Longhorn Driver, and {longhorn-product-name} UI) and system-managed components (for example, Instance Manager, Backing Image Manager, Share Manager, CSI Driver, and Engine Image). The following settings only apply to Longhorn Driver.
...
=== {longhorn-product-name} UI Settings

{longhorn-product-name} consists of user-deployed components (for example, Longhorn Manager, Longhorn Driver, and {longhorn-product-name} UI) and system-managed components (for example, Instance Manager, Backing Image Manager, Share Manager, CSI Driver, and Engine Image). The following settings only apply to {longhorn-product-name} UI.
...
| Affinity for {longhorn-product-name} UI pods. Specify the affinity you want to use for {longhorn-product-name} UI.

| `longhornUI.nodeSelector`
| object
| `{}`
| Node selector for {longhorn-product-name} UI. Specify the nodes allowed to run {longhorn-product-name} UI.

| `longhornUI.priorityClass`
| string
| `"longhorn-critical"`
| PriorityClass for {longhorn-product-name} UI.

| `longhornUI.replicas`
| int
| `2`
| Replica count for {longhorn-product-name} UI.

| `longhornUI.tolerations`
| list
| `[]`
| Toleration for {longhorn-product-name} UI on nodes allowed to run Longhorn components.
|===
...
| bool
| `false`
| Setting that allows {longhorn-product-name} to generate ingress records for the {longhorn-product-name} UI service.
...
| string
| `"/"`
| Default ingress path. You can access the {longhorn-product-name} UI by following the full ingress path `{\{host}}+{\{path}}`.
...
| bool
| `false`
| Setting that allows you to enable secure connections to the {longhorn-product-name} UI service via port 443.
...
| bool
| `false`
| Setting that allows {longhorn-product-name} to generate HTTPRoute records for the {longhorn-product-name} UI service using Gateway API.
...
| string
| `"/"`
| Default path for HTTPRoute. You can access the {longhorn-product-name} UI by following the full path.
...
You can install {longhorn-product-name} in an air-gapped environment with a private registry. For more information, see the *Air Gap Installation* section of the xref:installation-setup/installation/airgapped-environment.adoc[documentation].
...
| URL of a private registry. When unspecified, {longhorn-product-name} uses the default system registry.
...
| bool
| `false`
| Setting that allows {longhorn-product-name} to integrate with OpenShift.
...
| string
| `"longhorn-ui"`
| Route for connections between {longhorn-product-name} and the OpenShift web console.
|===
...
| `nil`
| Number of seconds that {longhorn-product-name} waits before checking the default backupstore for new backups. The default value is "300". When the value is "0", polling is disabled.

| `enableGoCoverDir`
| `false`
| Setting that allows {longhorn-product-name} to generate code coverage profiles.

| `enablePSP`
| `false`
| Setting that allows you to enable pod security policies (PSPs) that allow privileged {longhorn-product-name} pods to start. This setting applies only to clusters running Kubernetes 1.25 and earlier, and with the built-in Pod Security admission controller enabled.
...
| `true`
| Setting that allows {longhorn-product-name} to perform pre-upgrade checks. Disable this setting when installing {longhorn-product-name} using Argo CD or other GitOps solutions.

| `preUpgradeChecker.upgradeVersionCheck`
| `true`
| Setting that allows {longhorn-product-name} to perform upgrade version checks after starting the Longhorn Manager DaemonSet Pods. Disabling this setting also disables `preUpgradeChecker.jobEnabled`. {longhorn-product-name} recommends keeping this setting enabled.
|===

=== System Default Settings

During installation, you can either allow {longhorn-product-name} to use the default system settings or use specific flags to modify the default values. After installation, you can modify the settings using the {longhorn-product-name} UI. For more information, see the *Settings Reference* section of the xref:longhorn-documentation.adoc[documentation].

|===
| Key | Description

| `defaultSettings.allowCollectingLonghornUsageMetrics`
| Setting that allows {longhorn-product-name} to periodically collect anonymous usage data for product improvement purposes. {longhorn-product-name} sends collected data to the https://github.com/longhorn/upgrade-responder[Upgrade Responder] server, which is the data source of the Longhorn Public Metrics Dashboard (https://metrics.longhorn.io). The Upgrade Responder server does not store data that can be used to identify clients, including IP addresses.
...
| Setting that allows {longhorn-product-name} to automatically attach a volume and create snapshots or backups when recurring jobs are run.
...
| Setting that allows {longhorn-product-name} to automatically clean up the snapshot generated by a recurring backup job.
...
| Setting that allows {longhorn-product-name} to automatically clean up the system-generated snapshot after replica rebuilding is completed.

| `defaultSettings.autoDeletePodWhenVolumeDetachedUnexpectedly`
| Setting that allows {longhorn-product-name} to automatically delete a workload pod that is managed by a controller (for example, daemonset) whenever a Longhorn volume is detached unexpectedly (for example, during Kubernetes upgrades). After deletion, the controller restarts the pod and then Kubernetes handles volume reattachment and remounting.

| `defaultSettings.autoSalvage`
| Setting that allows {longhorn-product-name} to automatically salvage volumes when all replicas become faulty (for example, when the network connection is interrupted). {longhorn-product-name} determines which replicas are usable and then uses these replicas for the volume. This setting is enabled by default.

| `defaultSettings.backingImageCleanupWaitInterval`
| Number of minutes that {longhorn-product-name} waits before cleaning up the backing image file when no replicas in the disk are using it.

| `defaultSettings.backingImageRecoveryWaitInterval`
| Number of seconds that {longhorn-product-name} waits before downloading a backing image file again when the status of all image disk files changes to "failed" or "unknown".
...
| Maximum number of engines that are allowed to concurrently upgrade on each node after Longhorn Manager is upgraded. When the value is "0", {longhorn-product-name} does not automatically upgrade volume engines to the new default engine image version.
...
| Setting that allows {longhorn-product-name} to automatically create a default disk only on nodes with the label "node.longhorn.io/create-default-disk=true" (if no other disks exist). When this setting is disabled, {longhorn-product-name} creates a default disk on each node that is added to the cluster.

| `defaultSettings.csiAllowedTopologyKeys`
| Comma-separated list of topology keys that the {longhorn-product-name} CSI driver is allowed to pass through. When empty (default), no topology keys are passed through, and PVs will have no nodeAffinity. When configured (for example, `"topology.kubernetes.io/zone,topology.kubernetes.io/region"`), only the specified keys are kept in topology segments. All other keys are filtered out from both `CreateVolumeResponse.AccessibleTopology` and `NodeGetInfo` topology.

| `defaultSettings.csiStorageCapacityTracking`
...
| Default name of {longhorn-product-name} static StorageClass. `"storageClassName"` is assigned to PVs and PVCs that are created for an existing Longhorn volume. `"storageClassName"` can also be used as a label, so it is possible to use a Longhorn StorageClass to bind a workload to an existing PV without creating a Kubernetes StorageClass object. `"storageClassName"` needs to be an existing StorageClass. The default value is "longhorn-static".

| `defaultSettings.defaultReplicaCount`
| Default number of replicas for volumes created using the {longhorn-product-name} UI. For Kubernetes configuration, modify the `numberOfReplicas` field in the StorageClass. The default value is `"{"v1":"3","v2":"3"}"`.
...
| Flag that prevents accidental uninstallation of {longhorn-product-name}.
...
| Setting that disables the revision counter and thereby prevents {longhorn-product-name} from tracking all write operations to a volume. When salvaging a volume, {longhorn-product-name} uses properties of the `"volume-head-xxx.img"` file (the last file size and the last time the file was modified) to select the replica to be used for volume recovery. This setting applies only to volumes created using the {longhorn-product-name} UI.
...
| Number of minutes that {longhorn-product-name} keeps a failed backup resource. When the value is `"0"`, automatic deletion is disabled.
...
| Setting that notifies {longhorn-product-name} that the cluster is using the Kubernetes Cluster Autoscaler.
...
| Policy that defines the action {longhorn-product-name} takes when a volume is stuck with a StatefulSet or Deployment pod on a node that failed.

| `defaultSettings.nodeDrainPolicy`
| Policy that defines the action {longhorn-product-name} takes when a node with the last healthy replica of a volume is drained.
...
| PriorityClass for system-managed Longhorn components. This setting can help prevent Longhorn components from being evicted under Node Pressure. Notice that this is applied to {longhorn-product-name} user-deployed components by default if there are no priority class values set yet, such as `longhornManager.priorityClass`.
...
| Setting that allows {longhorn-product-name} to automatically mark the latest snapshot and its parent files as removed during a file system trim. {longhorn-product-name} does not remove snapshots containing multiple child files.
...
| Number of seconds that {longhorn-product-name} waits before reusing existing data on a failed replica instead of creating a new replica of a degraded volume.
...
| Setting that allows {longhorn-product-name} to schedule new replicas of a volume to nodes in the same zone as existing healthy replicas. Nodes that do not belong to any zone are treated as existing in the zone that contains healthy replicas. When identifying zones, {longhorn-product-name} relies on the label "topology.kubernetes.io/zone=+++<Zone name="" of="" the="" node="">+++" in the Kubernetes node object.+++</Zone>+++
...
| Setting that defines when {longhorn-product-name} checks the integrity of data in snapshot disk files. You must use the Unix cron expression format.
...
| Storage network for in-cluster traffic. When unspecified, {longhorn-product-name} uses the Kubernetes cluster network.
...
| Maximum number of failed support bundles that can exist in the cluster. When the value is "0", {longhorn-product-name} automatically purges all failed support bundles.
...
| Upgrade Checker that periodically checks for new {longhorn-product-name} versions. When a new version is available, a notification appears on the {longhorn-product-name} UI. This setting is enabled by default.

| `defaultSettings.upgradeResponderURL`
| The Upgrade Responder sends a notification whenever a new Longhorn version that you can upgrade to becomes available. The default value is `pass:[https://longhorn-upgrade-responder.rancher.io/v1/checkupgrade]`.
