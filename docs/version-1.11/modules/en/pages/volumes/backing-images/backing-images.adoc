...
[#_create_a_backing_image_using_a_storageclass_and_pvc]
...
Users can xref:_create_a_backing_image_using_a_storageclass_and_pvc[Create a Backing Image Using a StorageClass and PVC], directly create then immediately use a backing image via StorageClass or utilize an existing backing image as mentioned below.
...
. As mentioned in the section xref:_clean_up_backing_images_in_disks[Clean Up Backing Images in Disks], the file will be cleaned up automatically if all replicas in one disk do not use one backing image file.
