...
The helm chart install NetworkPolicy objects when the https://github.com/longhorn/longhorn/blob/v{patch-version}/chart/values.yaml[networkPolicies.enabled value] is set to `true`.
...
[IMPORTANT]
====
Depending on the deployed https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/[CNI], not all Kubernetes clusters support NetworkPolicy.
