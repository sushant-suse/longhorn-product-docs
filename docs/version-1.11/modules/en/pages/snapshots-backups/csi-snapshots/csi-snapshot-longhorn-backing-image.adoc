= CSI Volume Snapshot Associated with a SUSE® Storage Backing Image
...
BackingImage in {longhorn-product-name} is an object that represents a QCOW2 or RAW image which can be set as the backing/base image of a Longhorn volume.
...
*Result*: A Longhorn BackingImage is created. The `VolumeSnapshot` object creation leads to the creation of a `VolumeSnapshotContent` Kubernetes object.
...
[#_restore_pvc_from_csi_volumesnapshot_associated_with_longhorn_backingimage]
...
The csi-provisioner picks this up and instruct the Longhorn CSI driver to provision a new volume using the associated Longhorn BackingImage.
 ...
 ** `backingImageDataSourceType`: `sourceType` of existing BackingImage. For example, `export-from-volume`, `download`
 ** `backingImage`: Name of the BackingImage
 ** `backingImageChecksum`: It is optional. Checksum of the BackingImage.
...
The parameters can be retrieved from the menu:Advanced[Backing Images] page in the {longhorn-product-name} UI.
...
For an example see xref:_restore_pvc_from_csi_volumesnapshot_associated_with_longhorn_backingimage[Restore PVC from CSI VolumeSnapshot Associated With Longhorn BackingImage] above.
 ...
 ** `backingImageChecksum`: It is optional. Used for validating the file.
 ** example yaml:
+
[,yaml]
----
 ...
 ** `export-type`: Currently {longhorn-product-name} supports `raw` or `qcow2`
 ** example yaml:
+
[,yaml]
----
...
Now you can create a `PerstistantVolumeClaim` object that refers to the newly created `VolumeSnapshot` object. {longhorn-product-name} creates the BackingImage with the parameters provide in the `snapshotHandle`. For an example see xref:_restore_pvc_from_csi_volumesnapshot_associated_with_longhorn_backingimage[Restore PVC from CSI VolumeSnapshot Associated With Longhorn BackingImage] above.
