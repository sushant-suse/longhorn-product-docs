...
If you don't have access to AWS S3 or want to give the backupstore a try first, we've also provided a way to xref:_set_up_a_local_testing_backupstore[setup a local S3 testing backupstore] using https://minio.io/[MinIO].
...
NOTE: The secret can be named whatever you like as long as they match what is in {longhorn-product-name}'s settings.
...
If you want to use a self-signed SSL certificate, you can specify AWS_CERT in the Kubernetes secret you provided to {longhorn-product-name}. See the example in xref:_set_up_a_local_testing_backupstore[Set up a Local Testing Backupstore].
