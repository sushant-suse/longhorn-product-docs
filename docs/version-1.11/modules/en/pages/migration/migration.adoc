...
longhorn                                         	longhorn-system    	1       	2025-12-17 02:58:40.877484249 +0000 UTC	deployed	longhorn-107.2.0+up1.10.1                                                                               	v1.10.1
longhorn-crd                                     	longhorn-system    	1       	2025-12-17 02:58:38.192579711 +0000 UTC	deployed	longhorn-crd-107.2.0+up1.10.1                                                                           	v1.10.1
----
...
----
+
In this example, the release version is `107.2.0+up1.10.1`.

. Clone the `rancher/charts` repository containing the specific version of the `longhorn-crd` chart that matches your current release (for example, `107.2.0+up1.10.1` for Longhorn v1.10.1):
+
[,shell]
----
...
+
After running the command `bash ./download-longhorn-crd-chart.sh 107.2.0+up1.10.1`, the `longhorn-crd` chart will be downloaded to the local directory `107.2.0+up1.10.1`. Then, open `107.2.0+up1.10.1/Chart.yaml` to verify that the chart version matches `107.2.0+up1.10.1`.
...
[,shell]
----
bash ./patch-resource-policy-annotation.sh 107.2.0+up1.10.1/templates/crds.yaml
----
+
...
[,shell]
----
vimdiff /tmp/crds.yaml.original 107.2.0+up1.10.1/templates/crds.yaml
----
...
[,shell]
----
helm upgrade longhorn-crd -n longhorn-system ./107.2.0+up1.10.1
----
