...
The following example is migrating Longhorn {patch-version} installed via Rancher charts to {longhorn-product-name} {patch-version}.

[,shell]
----
$ helm list --all-namespaces | grep longhorn
longhorn                                         	longhorn-system    	1       	2025-12-17 02:58:40.877484249 +0000 UTC	deployed	108.2.0+up1.11.0                                                                               	v1.11.0
longhorn-crd                                     	longhorn-system    	1       	2025-12-17 02:58:38.192579711 +0000 UTC	deployed	longhorn-crd-108.2.0+up1.11.0                                                                           	v1.11.0
----
...
[,bash]
----
longhorn-crd                                     	longhorn-system    	1       	2025-12-17 02:58:38.192579711 +0000 UTC	deployed	longhorn-crd-108.2.0+up1.11.0+up1.11.0                                                                           	v1.11.0
----
+
In this example, the release version is `108.2.0+up1.11.0`.

. Clone the `rancher/charts` repository containing the specific version of the `longhorn-crd` chart that matches your current release (for example, `108.2.0+up1.11.0` for Longhorn v1.11.0):
+
[,shell]
----
...
+
After running the command `bash ./download-longhorn-crd-chart.sh 108.2.0+up1.11.0`, the `longhorn-crd` chart is downloaded to the local directory `108.2.0+up1.11.0`. Then, open `108.2.0+up1.11.0/Chart.yaml` to verify that the chart version matches `108.2.0+up1.11.0`.
...
[,shell]
----
bash ./patch-resource-policy-annotation.sh 108.2.0+up1.11.0/templates/crds.yaml
----
+
...
[,shell]
----
vimdiff /tmp/crds.yaml.original 108.2.0+up1.11.0/templates/crds.yaml
----
...
[,shell]
----
helm upgrade longhorn-crd -n longhorn-system ./108.2.0+up1.11.0
----
