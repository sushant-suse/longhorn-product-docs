...
xref:_automatic_volume_remounting[Automatic Volume Remounting] is triggered when the volume's filesystem changes to read-only. Longhorn remounts the global mount point on the host to change the state back to read-write.
