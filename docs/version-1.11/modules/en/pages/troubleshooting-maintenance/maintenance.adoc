...
stopped based on the xref:_node_drain_policy_recommendations[Node Drain Policy].
+
...
xref:nodes/disks-or-nodes-eviction.adoc[evict the replica to other nodes before draining]. See xref:_node_drain_policy_recommendations[Node Drain Policy
...
instance-manager pod that was running them will be stopped. Depending on the xref:_node_drain_policy_recommendations[Node Drain
...
replicas on other nodes. The replicas can be reused later, once the node back online and uncordoned. See xref:_node_drain_policy_recommendations[Node
...
xref:_updating_the_node_os_or_container_runtime[Updating the Node OS or Container Runtime].
