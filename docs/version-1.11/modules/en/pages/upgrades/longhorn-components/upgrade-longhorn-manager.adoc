...
. This installs `suse-storage` `{patch-version}`.
+
image::screenshots/install/update-repo.png[Update Repository, width=80%]
+
[NOTE]
====
Starting from Rancher Prime v2.13, you no longer need to manually edit the repository configuration to upgrade. You can simply go to *Apps > Charts*, select the `suse-storage` chart, and choose the target version from the *Chart Versions* panel before clicking *Upgrade to this version*.
====
...
. Update the URL from:
+
`oci://dp.apps.rancher.io/charts/suse-storage:<your-current-version>`
+
to:
+
`oci://dp.apps.rancher.io/charts/suse-storage:{patch-version}`
. Or, you can select `suse-storage` chart and choose the desired upgrade version.
. The repository should be updated successfully.
+
image::screenshots/install/updated-repo.png[Update Repository, width=80%]

. Go to *Apps > Installed Apps*.
. In the `longhorn-system` namespace, under the *Upgradable* column, look for a yellow-highlighted `{patch-version}` label. If you do not see the yellow label, refresh the page.
+
