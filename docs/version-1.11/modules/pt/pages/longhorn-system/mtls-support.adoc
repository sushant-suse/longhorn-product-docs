...
O Longhorn suporta mTLS para proteger e criptografar a comunicação gRPC entre o plano de controle (longhorn-manager) e o plano de dados (instance-managers).
Para a configuração do certificado, usamos o mecanismo de segredo do Kubernetes em combinação com uma montagem de segredo opcional para o longhorn-manager/instance-manager.

== Requisitos

Em uma instalação padrão, o mTLS está desativado; para habilitar o suporte ao mTLS, é necessário criar um segredo `longhorn-grpc-tls` no namespace `longhorn-system` antes da implantação.
O segredo é especificado como um ponto de montagem de segredo opcional para o longhorn-manager/instance-managers, portanto, se ele não existir quando esses componentes forem iniciados, o mTLS não será utilizado e será necessário reiniciar os componentes para habilitar o suporte ao TLS.

O longhorn-manager possui um fallback de cliente não TLS para configurações de modo misto, onde existem antigos instance-managers que foram iniciados sem suporte ao TLS.

== Configuração de certificado autoassinado

Você deve criar um `ca.crt` com a flag CA definida, que será usado para assinar o `tls.crt`. Isso permitirá que você renove o `tls.crt` no futuro sem interrupções no serviço.
Você pode usar https://mariadb.com/docs/server/security/securing-mariadb/securing-mariadb-encryption/data-in-transit-encryption/certificate-creation-with-openssl/[openssl] ou https://github.com/cloudflare/cfssl[cfssl] para o `ca.crt`, bem como para a geração do certificado `tls.crt`.

O certificado `tls.crt` deve usar `longhorn-backend` para o Nome Comum e a lista abaixo de entradas para o Nome Alternativo do Sujeito.
...
= Configuração de Segredos do Kubernetes

O `ca.crt` é o certificado da autoridade certificadora que foi usado para assinar o `tls.crt`, que será usado tanto pelo cliente (longhorn-manager) quanto pelo servidor (instance-manager) para autenticação gRPC mTLS.
A chave privada `tls.key` está associada ao `tls.crt` criado.

O yaml `longhorn-grpc-tls` se parece com o exemplo abaixo. Se você estiver tendo problemas para fazer seus próprios certificados funcionarem, pode decodificar o certificado abaixo e compará-lo com seus próprios certificados gerados via `openssl x509 -in tls.crt -text -noout`.
...
Para mais informações sobre como criar um segredo, consulte https://kubernetes.io/docs/concepts/configuration/secret/#creating-a-secret-manually[a documentação do Kubernetes.] O segredo deve ser criado no namespace `longhorn-system` para que o Longhorn possa acessá-lo.

NOTE: Certifique-se de usar `echo -n` ao gerar a codificação base64. Caso contrário, uma nova linha será adicionada ao final da string, o que causará um erro durante o carregamento dos certificados.

== Histórico

Disponível desde v1.3.0 https://github.com/longhorn/longhorn/issues/3839[#3839]
