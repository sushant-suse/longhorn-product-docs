...
The helm chart will install NetworkPolicy objects when the https://github.com/longhorn/longhorn/blob/v1.6.0/chart/values.yaml[networkPolicies.enabled value] is set to `true`.
The manifests of these objects can be viewed in the https://github.com/longhorn/longhorn/tree/v1.6.0/chart/templates/network-policies[git repository].
