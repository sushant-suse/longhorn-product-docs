...
List the resource types to be automatically deleted as a semicolon-separated string. Available types include:

* `replicaData`: Represents orphaned replica data directories on disk.
* `engineInstance`: Represents orphaned engine runtime processes.
* `replicaInstance`: Represents orphaned replica runtime processes.
