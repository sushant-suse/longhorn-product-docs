...
Specifies the wait time before {longhorn-product-name} automatically deletes an orphaned Custom Resource (CR) and the actual orphaned data or runtime instance it represents.

[NOTE]
====
If a user manually deletes an orphaned CR, the deletion occurs immediately and does not respect this grace period.
====
