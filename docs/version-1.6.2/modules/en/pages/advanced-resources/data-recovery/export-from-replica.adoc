...
+
More disks can be added to a node by either using the Longhorn UI or by using xref:nodes-and-volumes/nodes/default-disk-and-node-config.adoc[a node label and annotation].
+
