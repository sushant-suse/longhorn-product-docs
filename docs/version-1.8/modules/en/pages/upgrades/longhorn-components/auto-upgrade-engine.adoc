...
If a volume's engine fails to upgrade, the volume's status will not change. Because the status of the engine image will remain different from the spec, {longhorn-product-name} will retry the upgrade until it succeeds.

If there are too many volumes that fail to upgrade per node (i.e., more than the `concurrent automatic engine upgrade per node limit` setting), {longhorn-product-name} will stop upgrading volumes on that node.
