...
* Moving replicas or triggering automatic rebuilds on an unhealthy volume could further compromise data integrity. This design requires manual intervention so an administrator can assess the volume's condition before initiating potentially risky operations.
