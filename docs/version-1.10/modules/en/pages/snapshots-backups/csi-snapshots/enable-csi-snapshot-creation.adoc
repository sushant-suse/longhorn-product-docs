...
. Download the files from https://github.com/kubernetes-csi/external-snapshotter/tree/v8.4.0/client/config/crd
because {longhorn-product-name} v{patch-version} uses https://kubernetes-csi.github.io/docs/external-snapshotter.html[CSI external-snapshotter] v8.4.0
. Run `kubectl create -k client/config/crd`.
. Do this once per cluster.

=== Install the Common Snapshot Controller

. Download the files from https://github.com/kubernetes-csi/external-snapshotter/tree/v8.4.0/deploy/kubernetes/snapshot-controller (you can replace v{patch-version} with your desired {longhorn-product-name} version) because {longhorn-product-name} v{patch-version} uses https://kubernetes-csi.github.io/docs/external-snapshotter.html[CSI external-snapshotter] v8.4.0
