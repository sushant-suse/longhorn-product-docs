...
* `backupstore-poll-interval`: Number of seconds that {longhorn-product-name} waits before checking the default backupstore for new backups.
...
NOTE: If you operate {longhorn-product-name} behind a proxy and you want to use AWS S3 as the backupstore, you must provide {longhorn-product-name} information about your proxy in the `aws-secret` as below:
...
[NOTE]
====
The secret can be named whatever you like if they match what is in {longhorn-product-name}'s settings.
====
...
----
+
For `AWS_CERT` and `AWS_CERT_KEY` local test samples, see https://github.com/longhorn/longhorn/blob/v1.10.x/deploy/backupstores/minio-backupstore.yaml[MinIO backupstore manifest].
+
...
. Go to the {longhorn-product-name} UI. click *Backup and Restore/Backup Targets*, and create or edit a backup target.
+
Set *URL* to:
+
----
...
It is important to consider the certificate needs to be in PEM format, and must be its own CA. Or one must include a certificate chain that contains the CA certificate.
...
. You want to switch to this new access style right now so that you do not have to worry about https://aws.amazon.com/blogs/aws/amazon-s3-path-deprecation-plan-the-rest-of-the-story/[Amazon S3 Path Deprecation Plan];
...
For example, `nfsOptions=rw,nolock` becomes `rw,nolock,soft,timeo=300,retry=2` after {longhorn-product-name} applies its defaults. To extend the timeout, you can override `timeo` explicitly:
