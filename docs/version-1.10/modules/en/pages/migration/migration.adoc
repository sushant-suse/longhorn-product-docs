...
The following example is migrating **Longhorn v1.9.2 installed via Rancher charts to {longhorn-product-name} v1.9.2**.

[,shell]
----
$ helm list --all-namespaces | grep longhorn
longhorn                                         	longhorn-system    	1       	2025-09-24 02:58:40.877484249 +0000 UTC	deployed	longhorn-108.1.0+up1.9.2                                                                               	v1.9.2
longhorn-crd                                     	longhorn-system    	1       	2025-09-24 02:58:38.192579711 +0000 UTC	deployed	longhorn-crd-108.1.0+up1.9.2                                                                           	v1.9.2
----
...
----
+
In this example, the release version is `108.1.0+up1.9.2`.

. Clone the `rancher/charts` repository containing the specific version of the `longhorn-crd` chart that matches your current release (for example, `108.1.0+up1.9.2` for Longhorn v1.9.2):
+
[,shell]
----
...
+
After running the command `bash ./download-longhorn-crd-chart.sh 108.1.0+up1.9.2`, the `longhorn-crd` chart is downloaded to the local directory `108.1.0+up1.9.2`. Then, open `108.1.0+up1.9.2/Chart.yaml` to verify that the chart version matches `108.1.0+up1.9.2`.
...
[,shell]
----
bash ./patch-resource-policy-annotation.sh 108.1.0+up1.9.2/templates/crds.yaml
----
+
...
[,shell]
----
vimdiff /tmp/crds.yaml.original 108.1.0+up1.9.2/templates/crds.yaml
----
...
[,shell]
----
helm upgrade longhorn-crd -n longhorn-system ./108.1.0+up1.9.2
----
