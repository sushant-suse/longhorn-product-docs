...
* Regression:
** https://github.com/longhorn/longhorn/issues/12779[BUG-Backing image data source pod fails when HTTP proxy is enabled] that can cause failure of backing image upload when HTTP proxy is enabled.
...
. *Update the `backing-image-manager` image*
+
