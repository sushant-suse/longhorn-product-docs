...
The `longhorn-manager:v1.10.1` image is affected by the following issues:

* *Regression*
** https://github.com/longhorn/longhorn/issues/12233[[BUG\] Invalid memory address or nil pointer dereference] that can trigger a nil-pointer dereference under certain conditions, potentially causing unexpected crashes.
** https://github.com/longhorn/longhorn/issues/12311[[BUG\] Block Mode Volume Migration Stuck] that can cause block mode volume migration to get stuck indefinitely.

* *Day-one issues*
** *V2 volume clone*
*** https://github.com/longhorn/longhorn/issues/12328[[BUG\] V2 Volume CSI Clone Slowness Caused by VolumeAttachment Webhook] that can lead to significant delays during V2 volume cloning operations.
*** https://github.com/longhorn/longhorn/issues/12206[[BUG\] A cloned volume cannot be attached to a workload] that prevents cloned volumes from being attached to workloads.
** *Replica auto-balance*
*** https://github.com/longhorn/longhorn/issues/10837[[BUG\] Replica auto balance disk pressure threshold stalled with stopped volumes] that can cause the replica auto-balance feature to stall when volumes are stopped under disk pressure conditions.

To mitigate these issues, replace `longhorn-manager:v1.10.1` with the hotfixed image `longhorn-manager:v1.10.1-hotfix-2`.
...
The `longhorn-manager:v1.10.1` image is affected by a regression issue that can trigger a nil-pointer dereference under certain conditions and may cause unexpected crashes. To mitigate this issue, replace `longhorn-manager:v1.10.1` with the hotfixed image `longhorn-manager:v1.10.1-hotfix-1`. For details, see the related issue on GitHub.

Follow these steps to apply the update:

. Disable the upgrade version check.
* For Helm, set `upgradeVersionCheck` to `false` in the `values.yaml` file.
* For manifests, remove the `--upgrade-version-check` flag from the deployment manifest.

. Update the `longhorn-manager` image.
* Change the image tag from `v1.10.1` to `v1.10.1-hotfix-2` in the appropriate file.
** For Helm, update the `values.yaml` file.
** For manifests, update the deployment manifest.

. Apply the upgrade by running your standard Helm upgrade command or by applying the updated manifest.
...
For more details, see https://github.com/longhorn/longhorn/issues/11962[Issue #11962].
...
For more details, see https://github.com/longhorn/longhorn/issues/11816[Issue #11816].

=== UI Deployment Failure on IPv4-Only Nodes

Fixed a bug introduced in v1.10.0 where the {longhorn-product-name} UI failed to deploy on nodes with only IPv4 enabled. The UI now correctly supports IPv4-only configurations without requiring IPv6.

For more details, see https://github.com/longhorn/longhorn/issues/11875[Issue #11875].
...
For more details, see https://github.com/longhorn/longhorn/issues/12043[Issue #12043].
