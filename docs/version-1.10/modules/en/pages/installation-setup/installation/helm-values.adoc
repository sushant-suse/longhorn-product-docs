...
| bool
| `false`
| Setting that allows {longhorn-product-name} to run on a Rancher Windows cluster.
...
| Default data locality. A {longhorn-product-name} volume has data locality if a local replica of the volume exists on the same node as the pod that is using the volume.
...
| PriorityClass for system-managed Longhorn components. This setting can help prevent Longhorn components from being evicted under Node Pressure. Notice that this is be applied to {longhorn-product-name} user-deployed components by default if there are no priority class values set yet, such as `longhornManager.priorityClass`.
...
| Setting that allows you to configure maximum hugepage size (in MiB) for the V2 Data Engine.
