...
Specifies the plug-in that will be used for dynamic creation of persistent volumes. For Longhorn, that is always `"driver.longhorn.io"`.
...
____
Default: `ext4`
For more details, see xref:volumes/create-volumes.adoc#creating_longhorn_volumes_with_kubectl[Creating Longhorn Volumes with Kubernetes].
____
...
____
More details in xref:snapshots-backups/volume-snapshots-backups/create-recurring-backup-snapshot-job.adoc[Creating Recurring Backup and Snapshot Jobs].
____
