...
. Copy the https://github.com/longhorn/longhorn/blob/v{patch-version}/examples/restore_to_file.yaml.template[yaml template]: Make a copy of `examples/restore_to_file.yaml.template` as e.g. `restore.yaml`. You can replace v{patch-version} with your desired Longhorn version.
