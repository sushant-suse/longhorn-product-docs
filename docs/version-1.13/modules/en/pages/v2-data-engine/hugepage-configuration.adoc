...
The total huge page allocation on each node must be at least equal to the `data-engine-memory-size` value (default 2048 MiB). For detailed steps on enabling and verifying huge pages, see xref:v2-data-engine/hugepage-configuration.adoc[Enable HugePages].
