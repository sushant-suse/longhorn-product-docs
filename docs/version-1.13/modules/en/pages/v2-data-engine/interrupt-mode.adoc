...
{longhorn-product-name} supports *SPDK interrupt mode* for V2 data engine volumes as an alternative to the default *polling mode*.
...
| I/O handling
| Reactors continuously poll for work | I/O completions wake reactors through events

| Idle CPU usage
| Close to 100% on each allocated CPU core | Very low, with low-frequency background checks

| Latency
| Lowest and most predictable latency | Slightly higher under sustained high-throughput workloads

| Best fit
| High-performance workloads with frequent I/O | Resource-constrained clusters, fewer volumes, or sporadic I/O

| CPU allocation
| Uses CPUs selected by the V2 data engine CPU mask | Uses CPUs selected by the V2 data engine CPU mask
|===
...
For more information, see the official https://spdk.io/doc/system_configuration.html[SPDK documentation].
====
...
Interrupt mode uses the same V2 data engine CPU allocation settings as polling mode. {longhorn-product-name} still starts the SPDK target daemon (`spdk_tgt`) with an effective CPU mask, which determines the CPUs that SPDK reactors can use. For CPU mask syntax, calculation examples, global settings, and per-node overrides, see xref:v2-data-engine/configurable-cpu-cores.adoc[Configurable CPU Cores].
...
| CPUs available to `spdk_tgt`
| CPUs selected by the effective CPU mask
| CPUs selected by the effective CPU mask

| Idle behavior
| Reactors continuously busy-poll
| Reactors can block while waiting for I/O events or low-frequency checks

| Idle CPU usage
| Close to 100% on each masked CPU
| Very low, but not zero

| Peak CPU usage
| Bounded by the masked CPUs
| Bounded by the masked CPUs
|===
...
* *Keep the effective Instance Manager CPU request aligned* with the CPU mask. Even though idle usage is low in interrupt mode, the reactors still need those CPUs during I/O bursts. See xref:longhorn-system/settings.adoc#_guaranteed_instance_manager_cpu[Guaranteed Instance Manager CPU].
