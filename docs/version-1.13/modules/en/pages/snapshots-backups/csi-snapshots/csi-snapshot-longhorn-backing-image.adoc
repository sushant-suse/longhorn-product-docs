...
BackingImage in {longhorn-product-name} is an object that represents a QCOW2 or RAW image which can be set as the backing or base image of a Longhorn volume.
...
{longhorn-product-name} creates the BackingImage with the parameters provide in the `snapshotHandle`.
For an example see xref:_restore_pvc_from_csi_volumesnapshot_associated_with_longhorn_backingimage[Restore PVC from CSI VolumeSnapshot Associated With Longhorn BackingImage] above.
