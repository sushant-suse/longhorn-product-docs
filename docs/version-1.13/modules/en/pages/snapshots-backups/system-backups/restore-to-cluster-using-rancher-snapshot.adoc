= Restore the SUSE® Storage System to a Cluster Using a Rancher Snapshot
...
* The setting xref:longhorn-system/settings.adoc#_disable_revision_counter[`Disable Revision Counter`] is false. (It is false by default.) Otherwise, users need to manually check if the data among volume replicas are consistent, or directly restore volumes from backup.
...
* Tou have to restart the Kubernetes components on all nodes. Otherwise, there will be tons of resource update conflicts in Longhorn.
 ...
 ** A volume is unchanged: Users do not need to do anything.
 ** The data is updated: Users do not need to do anything typically. Longhorn will automatically fail the replicas that do not contain the latest data.
 ...
 ** For DR volumes: Users do not need to do anything. Longhorn will redo a full restore.
 ...
 ** *Notice*: If users do not know how to recover a problematic volume, the simplest way is always restoring a new volume from backup.
...
* If a node is deleted from Longhorn system after the snapshot, the node would not be back, but the pods on the removed node will be restored. Users need to manually clean up them since these pod may get stuck in state `Terminating`.
...
In this GitHub post, one user is providing a way that restores the Longhorn to a new cluster that does not contain any data.
