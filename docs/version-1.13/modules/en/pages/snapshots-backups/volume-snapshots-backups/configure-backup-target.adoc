...
A backup target is an endpoint used to access a backupstore. Backup targets can be configured on the {longhorn-product-name} UI (*Settings > Backup Target*). A backupstore is a server that stores the backups of Longhorn volumes. You can use NFS, SMB or CIFS, Azure Blob Storage, and S3-compatible servers.

[NOTE]
====
Starting with v1.8.0, {longhorn-product-name} supports usage of multiple backupstores. Setting the default backup target before creating a new one is recommended.
====
...
For more information about how the backupstore works in {longhorn-product-name}, see xref:introduction/concepts.adoc#_3_backups_and_secondary_storage[Concepts].

If you do not have access to AWS S3 or want to give the backupstore a try first, we have also provided a way to xref:_set_up_a_local_testing_backupstore[setup a local S3 testing backupstore] using https://minio.io/[MinIO].

{longhorn-product-name} also supports setting up recurring snapshot or backup jobs for volumes, via {longhorn-product-name} UI or Kubernetes Storage Class. See xref:snapshots-backups/volume-snapshots-backups/create-recurring-backup-snapshot-job.adoc[here] for details.

[NOTE]
====
* The lifecycle of Longhorn backups within the backupstore is managed entirely by {longhorn-product-name}. *Applying retention policies directly on the backupstore is strictly prohibited*.

* {longhorn-product-name} attempts to clean up backup-related custom resources in the following scenarios:
...
* `defaultBackupStore.pollInterval`: Number of seconds that {longhorn-product-name} waits before checking the default backupstore for new backups.
...
----
 ...
 .. Create a Kubernetes secret with a name such as `aws-secret` in the namespace where {longhorn-product-name} is placed (`longhorn-system` by default). The secret must be created in the `longhorn-system` namespace for {longhorn-product-name} to access it:
+
...
. On the {longhorn-product-name} UI, go to *Backup and Restore > Backup Targets*, and then create or edit a backup target.
+
Set *URL* to:
+
...
----
+
Also make sure you have set *`<your-aws-region>` in the URL*.
+
...
[NOTE]
====
If you operate {longhorn-product-name} behind a proxy and you want to use AWS S3 as the backupstore, you must provide {longhorn-product-name} information about your proxy in the `aws-secret` as below:
====
...
Make sure `NO_PROXY` contains the network addresses, network address ranges and domains that should be excluded from using the proxy. For {longhorn-product-name} to operate, the minimum required values for `NO_PROXY` are:
...
+
The serviceaccount requires the `roles/storage.objectAdmin` role to read, write and delete objects in the bucket.
+
...
+
Also note down the configured _Storage URI_ under the _Request Endpoint_ while you are in the interoperability menu.

* The Access Key is mapped to the `AWS_ACCESS_KEY_ID` field in the Kubernetes secret we create later.
* The Secret is mapped to the `AWS_SECRET_ACCESS_KEY` field in the Kubernetes secret we create later.
* The Storage URI is mapped to the `AWS_ENDPOINTS` field in the Kubernetes secret we create later.

. Go to the {longhorn-product-name} UI. In the top navigation bar, click *Backup and Restore or Backup Targets*, and create or edit a backup target.
+
Set *URL* to:
+
----
...
[NOTE]
====
The secret can be named whatever you like. Make sure that they match what is in {longhorn-product-name}'s settings.
====

Once the secret is created and {longhorn-product-name}'s settings are saved, navigate to the backup tab in {longhorn-product-name}. If there are any issues, they should pop up as a toast notification.

If you do not get any error messages, try creating a backup and confirm the content is pushed out to your new bucket.
...
[#_set_up_a_local_testing_backupstore]
...
----
+
For `AWS_CERT` and `AWS_CERT_KEY` local test samples, see https://github.com/longhorn/longhorn/blob/v{patch-version}/deploy/backupstores/minio-backupstore.yaml[MinIO backupstore manifest].
+
...
. Go to the {longhorn-product-name} UI. Click *Backup and Restore or Backup Targets*, and create or edit a backup target.
+
Set *URL* to:
+
----
...
[NOTE]
====
Make sure to use `echo -n` when generating the base64 encoding, otherwise a new line will be added at the end of the string and it will cause error when accessing the S3.
====
...
To use a self-signed SSL certificate, you can specify AWS_CERT in the Kubernetes secret you provided to {longhorn-product-name}. See the example in xref:_set_up_a_local_testing_backupstore[Set up a Local Testing Backupstore].
...
. You want to switch to this new access style right now so that you do not need to worry about https://aws.amazon.com/blogs/aws/amazon-s3-path-deprecation-plan-the-rest-of-the-story/[Amazon S3 Path Deprecation Plan];
. The backupstore you are using supports virtual-hosted-style access only, for example, Alibaba Cloud(Aliyun) OSS;
...
. Add a new field `VIRTUAL_HOSTED_STYLE` with value `true` to your backup target secret. For example:
+
[subs="+attributes",yaml]
----
apiVersion: v1
...
. Create the corresponding backup target in *Backup and Restore > Backup Targets* by providing the following details:
...
If `nfsOptions` is not set, {longhorn-product-name} uses the default mount options `actimeo=1,soft,timeo=300,retry=2`.
...
When `nfsOptions` is specified, {longhorn-product-name} adjusts the options to ensure NFS operations fail instead of hanging indefinitely when the NFS service is unstable:
...
Before configuring a SMB or CIFS backupstore, a credential secret for the backupstore can be created and deployed by
...
On the {longhorn-product-name} UI, go to *Backup and Restore > Backup Targets*.
...
+
. Go to the {longhorn-product-name} UI. In the top navigation bar, click *Backup and Restore/Backup Targets*, and create or edit a backup target.
+
...
+
This secret is used to access the backupstore in the {longhorn-product-name} namespace (default: `longhorn-system`) with the following content:
+
[,shell]
----
#!/bin/bash
