...
By default, {longhorn-product-name} does not automatically delete orphaned instances through this manual UI operation alone. To enable automatic deletion of orphaned instances, or to configure settings related to general orphaned data, navigate to *Settings > Orphan* and configure the relevant options. (Refer to the `kubectl` section for details on the specific `orphan-resource-auto-deletion` setting, for instance, CRs if managing via back-end settings).
