...
. Use the `lsof` command to make sure no one is currently using the volume, for example:
+
----
...
----
+
The above result shows that the data directory is still being used, so do not proceed to the next step. If it is not being used, `lsof` command should return empty result.
...
____
To avoid accidental change of the volume content, it is recommended to use `mount -o ro` to mount the directory as `readonly`.
____
