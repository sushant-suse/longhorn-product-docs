...
{longhorn-product-name} relies heavily on kernel functionality and performs better on certain kernel versions. The following activities, in particular, benefit from usage of specific kernel versions.

* Optimizing or improving the file system: Use a kernel with version `v5.8` or later. See https://github.com/longhorn/longhorn/issues/2507#issuecomment-857195496[Issue #2507] for details.
* Enabling the xref:longhorn-system/settings.adoc#_freeze_filesystem_for_snapshot[Freeze Filesystem for Snapshot] setting: Use a kernel with version `5.17` or later to ensure that a volume crash during a file system freeze cannot lock up a node.
* Enabling the xref:installation-setup/requirements.adoc#_v2_data_engine_requirements[V2 Data Engine]: Use a kernel with version `5.19` or later to ensure NVMe or TCP support. Use a kernel with version `6.7` or later for improved stability (avoids potential memory corruption from SPDK upstream issue https://github.com/spdk/spdk/issues/3116#issuecomment-1890984674[#3116]).
...
=== CNI Plug-in Compatibility

{longhorn-product-name} v{patch-version} has been verified with https://github.com/longhorn/longhorn/blob/0c1d5ab907b0d7fa6232e05f9cb4ef6733cf7cbe/chart/values.yaml#L39[`restrictInternalTraffic`] enabled, which enforces network policies for internal {longhorn-product-name} components so that only authorized components can communicate with each other, against the following Kubernetes distribution and CNI plugin combinations.

[options="header"]
|===
| Kubernetes Distribution | CNI Plug-in 
| K3s                     | Flannel 
| K3s                     | Calico 
| K3s                     | Cilium 
| RKE2                    | Canal 
...
* *Ensure disk is clean before adding*: Starting with v1.11.0, {longhorn-product-name} prevents adding block disks that contain an existing file system or partition table. Run `wipefs -a /path/to/block/device` before adding a disk.
...
* *Huge pages configuration*: Ensure 2 GiB of 2 MiB-sized huge pages (1024 pages) are configured persistently on each V2 node via kernel boot parameters. See xref:installation-setup/requirements.adoc#_enable_hugepages[Enable HugePages] for detailed instructions.
* *Kernel modules*: Ensure `vfio_pci`, `uio_pci_generic`, and `nvme-tcp` modules are loaded and configured to load automatically at boot. See xref:installation-setup/requirements.adoc#_load_kernel_modules[Load Kernel Modules].

For the complete V2 Data Engine setup guide, see xref:installation-setup/requirements.adoc#_v2_data_engine_requirements[V2 Data Engine Requirements].
...
{longhorn-product-name} create snapshots automatically when rebuilding a replica. Recurring snapshots or backups can also automatically clean up the system-generated snapshot.
...
The primary CPU consumer in the pod is the Storage Performance Development Kit (SPDK) target daemon (`spdk_tgt`). The xref:longhorn-system/settings.adoc#_data_engine_cpu_mask[Data Engine CPU Mask] and xref:longhorn-system/settings.adoc#_data_engine_number_of_cpu_cores[Data Engine Number of CPU Cores] settings control how many CPU cores `spdk_tgt` uses.
...
The primary CPU consumer in the pod is the Storage Performance Development Kit (SPDK) target daemon (`spdk_tgt`). By default, `spdk_tgt` typically uses 1 dedicated CPU core in polling mode. The xref:longhorn-system/settings.adoc#_data_engine_cpu_mask[Data Engine CPU Mask] setting controls which CPU cores `spdk_tgt` runs on.
...
Avoid modifying the default StorageClass named `longhorn`. Changing its parameters can cause issues during future upgrades. To change the parameters set in the StorageClass, you can create a new StorageClass by referring to the xref:longhorn-system/examples-resources.adoc#_storageclass[StorageClass examples].
