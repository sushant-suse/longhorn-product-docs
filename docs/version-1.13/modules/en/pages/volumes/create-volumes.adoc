...
For help creating volumes from the {longhorn-product-name} UI, see the Creating Longhorn Volumes with the xref:_creating_longhorn_volumes_with_the_ui[{longhorn-product-name} UI section].
...
{longhorn-product-name} supports the following Kubernetes PersistentVolume access modes:
...
* *ReadWriteMany (RWX)*: The volume can be mounted as read-write by many nodes at the same time, enabling shared access across multiple pods. For more information, see xref:volumes/rwx-volumes.adoc[ReadWriteMany (RWX) volumes].
...
[#_creating_v1_longhorn_volumes_with_kubectl]
=== Creating V1 Longhorn Volumes with kubectl

Before creating a V1 volume, ensure that Longhorn has at least one available file system-type disk. V1 volumes are scheduled only to file system-type disks. For more information, see xref:nodes/multiple-disks.adoc#_add_a_filesystem-type_disk[Add a Filesystem-Type Disk].
...
[#_creating_v2_longhorn_volumes_with_kubectl_replication]
=== Creating V2 Longhorn Volumes with kubectl (Replication)

Before creating a V2 volume, ensure that the V2 Data Engine is enabled and Longhorn has available block-type disks. V2 volumes are scheduled only to block-type disks. For more information, see xref:v1-v2-volume-behavior-and-feature-parity.adoc[V2 Data Engine Guide] and xref:nodes/multiple-disks.adoc#_add_a_block-type-disk[Add a Block-Type Disk].

. Use the following command to create a StorageClass called `longhorn-v2-replicated`:
+
[,shell]
----
kubectl create -f https://raw.githubusercontent.com/longhorn/longhorn/v{patch-version}/examples/v2/replication/storageclass.yaml
----
+
The following example StorageClass is created:
+
...
----
+
The `dataEngine` parameter must be set to `v2` so Longhorn provisions a V2 volume. See xref:volumes/storageclass-parameters.adoc[Storage Class Parameters] for the list of supported parameters.

. Create a Pod that uses a V2 Longhorn volume by running this command:
+
[,shell]
----
kubectl create -f https://raw.githubusercontent.com/longhorn/longhorn/v{patch-version}/examples/v2/replication/pod_with_pvc.yaml
----
+
...
More examples are available xref:longhorn-system/examples-resources.adoc[examples section].

[#_creating_v2_longhorn_volumes_with_kubectl_sharding]
=== Creating V2 Longhorn Volumes with kubectl (sharding)

[NOTE]
====
Sharding is an Experimental feature. It is intended for evaluation and testing, and is not recommended for production use.
====

Sharding distributes a volume's data across multiple nodes using erasure coding, allowing a volume to exceed the capacity of a single disk or node. It is an alternative to the default replicated layout and is only available with the V2 Data Engine. As with any V2 volume, ensure that the V2 Data Engine is enabled and Longhorn has available block-type disks. Besides that, each chunk is placed on a distinct node, so the cluster must have at least `dataChunks + parityChunks` schedulable nodes. For more information, see xref:nodes/multiple-disks.adoc#_add_a_block_type_disk[Add a Block-Type Disk].

. Use the following command to create a StorageClass called `longhorn-v2-sharded`:
+
[source,shell]
----
kubectl create -f https://raw.githubusercontent.com/longhorn/longhorn/v{patch-version}/examples/v2/sharding/storageclass.yaml
----
+
...
----
+
The `dataLayout.*` parameters define the erasure-coded layout. See xref:volumes/storageclass-parameters.adoc[Storage Class Parameters] for the full list.

. Create a Pod that uses the sharded volume by running this command:
+
[source,shell]
----
kubectl create -f https://raw.githubusercontent.com/longhorn/longhorn/v{patch-version}/examples/v2/sharding/pod_with_pvc.yaml
----
+
...
For information about data layout parameters, supported and unsupported features, and how sharding compares to replication, see xref:v2-data-engine/sharding.adoc[Sharding with Erasure Coding].
...
[#_creating_longhorn_volumes_with_the_ui]
...
Users need to manually delete PVC and PV created by {longhorn-product-name}.
