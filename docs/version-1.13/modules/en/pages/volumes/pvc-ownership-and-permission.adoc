 ...
 ** If empty, the Kubelet does not attempt to change the ownership and permission for the volume.
. Check `fsType` of the PV and `accessModes` of the PVC.
 ** If `fsType` of the PV is defined and `accessModes` of the PVC list contains `ReadWriteOnce`, continue to the next step.
 ** Otherwise, the Kubelet does not attempt to change the ownership and permission for the volume.
 ...
 ** If the `pod.spec.securityContext.fsGroupChangePolicy` is set to `OnRootMismatch`:
  *** If the root of the volume already has the correct permissions (that is, belongs to the group id as `pod.spec.securityContext.fsGroup`) , the recursive permission and ownership change will be skipped.
