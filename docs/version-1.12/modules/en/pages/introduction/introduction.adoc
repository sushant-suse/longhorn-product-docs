...
== Assign Multiple Storage Front-ends for Each Volume
...
Specify the frequency of these operations (hourly, daily, weekly, monthly and yearly), the exact time at which these operations are performed (for example, 3:00am every Sunday), and how many recurring snapshots and backup sets are kept.
