...
. To create the backing image, as long as it does not exist during the CSI volume creation, parameters `backingImageDataSourceType` and `backingImageDataSourceParameters` should be set. Similarly to YAML, it is better not to create a backing image via "upload" in StorageClass. Note that if all these parameters are set and the backing image already exists, {longhorn-product-name} validates if the parameters match the existing one before using it.
...
parameters:
  ...
  dataEngine: "v1"
----
. Create a PVC with the StorageClass. Then the backing image is created (with the {longhorn-product-name} volume) if it does not exist.
...
Users can xref:_create_a_backing_image_using_a_storageclass_and_pvc[Create a Backing Image Using a StorageClass and PVC], or utilize an existing backing image as mentioned below.
...
[#_download_a_backing_image_file]
...
[#_clean_up_backing_images_in_disks]
...
* If there is still a ready backing image file on one disk, {longhorn-product-name} automatically clean up the failed backing image files, then re-launch these files from the ready one.
* If somehow all files of a backing image become failed, and the first file is :
 ** downloaded from a URL, {longhorn-product-name} restarts the downloading.
 ** exported from an existing volume, {longhorn-product-name} (attach the volume if necessary then) restarts the export.
...
* When a node is down or the backing image manager pod on the node is unavailable, all backing image files on the node becomes `unknown`. Later, if the node is back and the pod is running, {longhorn-product-name} detects that and then reuse the existing files automatically.
...
. Once a backing image file is prepared by the backing image manager for a disk, the file is shared among all volume replicas in this disk.
...
. As mentioned in the section xref:_clean_up_backing_images_in_disks[clean up backing images in disks], the file is cleaned up automatically if all replicas in one disk do not use one backing image file.
...
* The download URL of the backing image should be public. It will be improved in future.
* If there is high memory usage of one backing image manager pod after xref:_download_a_backing_image_file[file download] this is caused by the system cache or buffer. The memory usage will decrease automatically hence you do not need to worry about it. See https://github.com/longhorn/longhorn/issues/4055[the GitHub ticket] for more details.
