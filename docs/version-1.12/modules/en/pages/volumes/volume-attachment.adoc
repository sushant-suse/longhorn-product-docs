...
* `csi-attacher`: The most common type, handling standard attachment requests from the Kubernetes CSI plug-in, typically when mounting a volume to a pod.
...
* `share-manager-controller`: Manages back-end volume attachments for ReadWriteMany (RWX) volumes by attaching them to the share-manager pod.
