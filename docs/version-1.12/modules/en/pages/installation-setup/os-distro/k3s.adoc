...
This error is due to {longhorn-product-name} cannot detect where is the root dir setup for Kubelet, so the CSI plug-in installation failed.
...
Run `ps aux | grep k3s` and get argument `--data-dir` or `-d` on k3s node. For example, the result could be like below:
...
=== {longhorn-product-name} versions earlier than v0.7.0 do not work on K3s v0.10.0 or later
