...
[NOTE]
====
After changing this setting, you must manually restart the `longhorn-csi-plugin` DaemonSet for the change to take effect. Topology is applied correctly only after the CSI plug-in pod on each node has restarted.
====
