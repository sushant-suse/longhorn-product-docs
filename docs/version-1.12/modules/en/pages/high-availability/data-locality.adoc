...
* `best-effort`: This option instructs Longhorn to try to keep a replica on the same node as the attached volume (workload). Longhorn will not stop the volume, even if it cannot keep a replica local to the attached volume (workload) due to an environment limitation, for example, not enough disk space, incompatible disk tags, etc.
...
It does not change any existing volume's settings.
