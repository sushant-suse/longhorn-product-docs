...
* When offline replica rebuilding is triggered, {longhorn-product-name} attaches the volume without activating the front-end, rebuilds any missing replicas, and then detaches the volume after the rebuilding process is complete.
...
When offline rebuilding starts, degraded volumes can get stuck in the attached state if rebuilding conditions are not met. To prevent this, if the necessary conditions are not satisfied, offline rebuilding would not start or will be canceled.

* *Benefits*:
** It ensures volumes do not remain stuck in the attached state if rebuilding never finishes.
