...
See xref:_full_replica_rebuilding[full replica rebuilding].
...
*** If _yes_, {longhorn-product-name} skips transferring the data of the snapshot. This optimization reduces CPU usage, disk I/O, network I/O, and total rebuild time. See xref:_fast_replica_rebuilding[fast replica rebuilding].
...
See xref:_delta_replica_rebuilding[delta replica rebuilding].
...
If a volume head snapshot is created but its checksum is not ready, {longhorn-product-name} must perform delta rebuilding.  
...
* *If fast replica rebuilding is enabled and all snapshot checksums exist*:  
  {longhorn-product-name} triggers xref:_fast_replica_rebuilding[fast replica rebuilding].
...
* *If fast replica rebuilding is enabled but some snapshot checksums are missing*:  
  {longhorn-product-name} triggers xref:_delta_replica_rebuilding[delta replica rebuilding]..  
...
--
* `enabled`:  
  Skips snapshot data transfer if checksums are current. Provides fast rebuilds but does not revalidate data.
