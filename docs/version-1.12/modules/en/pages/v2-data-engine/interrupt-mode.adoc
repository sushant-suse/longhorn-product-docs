...
Interrupt mode is particularly suitable for clusters with limited CPU resources and a small number of volumes. While polling mode maximizes performance by keeping CPU utilization close to 100% on allocated cores, interrupt mode reduces CPU usage by allowing the SPDK reactor to adjust its usage dynamically instead of continuously polling.
...
[NOTE]
====
IOMMU support may not be exposed on virtualized instances. If unsure, consider using a bare metal instance, or consult documentation or support team of your cloud provider.
====
+
...
The current V2 volume interrupt mode uses a hybrid approach for NVMe or TCP transport:
