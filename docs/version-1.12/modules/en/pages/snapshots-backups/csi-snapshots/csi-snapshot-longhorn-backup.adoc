= CSI Volume Snapshot Associated with a SUSE® Storage Backup
:revdate: 2026-06-01
...
The CSI snapshotter sidecar periodically queries the Longhorn CSI plug-in to evaluate the backup status.
...
For an example see xref:_restore_pvc_from_csi_volumesnapshot_associated_with_longhorn_backup[Restore PVC from CSI VolumeSnapshot Associated With Longhorn Backup] above.
