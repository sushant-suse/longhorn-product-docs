...
When the number of snapshots on a volume reaches this threshold, {longhorn-product-name} sets the `TooManySnapshots` condition. This threshold is independent of the hard limit enforced by `snapshot-max-count`. See <<count-based-threshold, Count-Based Threshold>>.
...
When `SnapshotMaxSize` is set to a non-zero value, {longhorn-product-name} also uses it as the threshold for the `TooManySnapshots` condition. See <<toomanysnapshots-condition, TooManySnapshots Condition>>.
