...
{longhorn-product-name} can easily restore backups to a volume.
...
When you restore a backup, it creates a volume of the same name by default. If a volume with the same name as the backup already exists, the backup cannot be restored.

[NOTE]
====
*Cross-Engine Restore (Offline Volume Migration)*: {longhorn-product-name} fully supports restoring a backup across different data engines. For example, you can back up a V1 volume and restore it as a V2 volume. This serves as an effective method for offline volume migration from the V1 data engine to the V2 Data Engine.
====

== Restore via {longhorn-product-name} UI
...
4.  In the *Data Engine* drop-down menu, select your desired target data engine (for example, select `v2` to migrate a backed-up V1 volume to the V2 data engine).
5.  Click *OK*.

You can then create the PV or PVC from the volume after restoring a volume from a backup. Here you can specify the `storageClassName` or leave it empty to use the `storageClassName` inherited from the PVC of the backup volume. The `StorageClass` should already be in the cluster to prevent any further issues.
...
[IMPORTANT]
====
The `size` field must be the exact byte count provided by the backup status, wrapped in *double quotes* (for example, `"1073741824"`). Using Kubernetes quantities like `1Gi` or raw integers without quotes result in an admission webhook error.
====

To perform an offline volume migration to a different data engine during the restore process, simply set the `dataEngine` field to your target engine (for example, `v2`).
...
spec:
  ...
  numberOfReplicas: 3
  frontend: blockdev
  dataEngine: v2 # Change this to v2 to migrate a v1 backup to the v2 data engine
----
