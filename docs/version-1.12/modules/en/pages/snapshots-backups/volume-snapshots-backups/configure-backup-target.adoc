...
A backup target is an endpoint used to access a backupstore. Backup targets can be configured on the {longhorn-product-name} UI (*Settings > Backup Target*). A backupstore is a server that stores the backups of Longhorn volumes. You can use NFS, SMB/CIFS, Azure Blob Storage, and S3-compatible servers.
...
If you do not have access to AWS S3 or want to give the backupstore a try first, we have also provided a way to <<_set_up_a_local_testing_backupstore,setup a local S3 testing backupstore>> using https://minio.io/[MinIO].
