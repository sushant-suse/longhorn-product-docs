...
____
*Prerequisite:* A backup target must be set up. For more information, see xref:snapshots-backups/volume-snapshots-backups/configure-backup-target.adoc[Set the BackupTarget]. If the BackupTarget has not been set, it will be presented with an error.
____
