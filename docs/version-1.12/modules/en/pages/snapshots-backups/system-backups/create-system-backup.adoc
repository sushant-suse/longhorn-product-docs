...
[NOTE]
====
Unsetting the backup target clears the existing `SystemBackup` custom resource. Longhorn syncs to the remote backup store after setting the backup target. Another cluster can also sync to the same list of system backups when the backup target is the same.
====
...
[NOTE]
====
Longhorn system restores volume with the latest backup. We recommend updating the last backup for all volumes. By taking volume backups, you ensure that the data is up-to-date with the system backup. For more information, please refer to the xref:_volume_backup_policy[Configuration - Volume Backup Policy] section.
====

=== Configuration

[#_volume_backup_policy]
==== Volume Backup Policy

The Longhorn system backup offers the following volume backup policy options:

* `if-not-present`: {longhorn-product-name} creates a backup for volumes that either lack an existing backup or have an outdated latest backup.
* `always`: {longhorn-product-name} creates a backup for all volumes, regardless of their existing backups.
* `disabled`: {longhorn-product-name} does not create any backups for volumes.
...
. The system backup is ready to use when the state changes to `Ready`.
...
You can delete the Longhorn system backup in the remote backup target using the {longhorn-product-name} UI. Or with the `kubectl` command.

=== Delete a System Backup Using the {longhorn-product-name} UI

. Go to the `System Backup` page in the `Setting` drop-down list.
. Delete a single system backup in the `Operation` drop-down list next to the system backup. Or delete in batch with the `Delete` button.
+
[NOTE]
====
Deleting the system backup also makes a deletion in the backup store.
====
