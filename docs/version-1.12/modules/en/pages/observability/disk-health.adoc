...
Starting with {longhorn-product-name} v1.11.0, disk health monitoring metrics are available for both the V1 and V2 data engines. These metrics provide insights into disk health status.

[NOTE]
====
* {longhorn-product-name} collects health data every 10 minutes.
...
* SMART attributes: https://www.smartmontools.org/wiki/TocDoc[smartmontools documentation]
* SPDK NVMe health data: https://github.com/spdk/spdk/blob/v25.09/doc/jsonrpc.md.jinja2#L4805-L4856[`bdev_nvme_get_controller_health_info` JSON-RPC]
====
...
* Related GitHub issue https://github.com/longhorn/longhorn/issues/12016[#12016].
