...
* Longhorn Back-end service is a service pointing to the set of Longhorn manager pods. Longhorn's metrics are exposed in Longhorn manager pods at the endpoint `+http://LONGHORN_MANAGER_IP:PORT/metrics+`.
...
[NOTE]
====
ServiceMonitor provides endpoint discovery, but does not automatically authorize network traffic to Longhorn Manager pods.
====
...
* `networkPolicies.enabled` now controls _only_ the UI front-end policy.
====
...
+
____
Use the above `NodePort` service for quick verification only because it does not communicate over the TLS connection. You may want to change the service type to `ClusterIP` and set up an Ingress-controller to expose the web UI of Alertmanager over a TLS connection.
____
...
+
____
Use the above NodePort service for quick verification only because it does not communicate over the TLS connection. You may want to change the service type to `ClusterIP` and set up an Ingress controller to expose the web UI of the Prometheus server over a TLS connection.
____
...
+
____
Use the above NodePort service for quick verification only because it does not communicate over the TLS connection. You may want to change the service type to ClusterIP and set up an Ingress controller to expose Grafana over a TLS connection.
____
