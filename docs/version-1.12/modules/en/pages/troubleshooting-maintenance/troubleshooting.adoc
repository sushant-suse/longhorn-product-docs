...
For a more in-depth troubleshooting flow please see https://github.com/longhorn/longhorn/wiki/Troubleshooting[{longhorn-product-name} Troubleshooting].

There are a few components in {longhorn-product-name} - Manager, Engine, Driver and UI. By default, all those components run as pods in the `longhorn-system` namespace in the Kubernetes cluster.
...
Making use of the {longhorn-product-name} UI is a good start for troubleshooting. For example, if Kubernetes cannot mount a volume correctly, after stopping the workload, try to attach and mount that volume manually on one node and access the content to check if the volume is intact.

Also, the event logs in the UI dashboard provide certain information on probable issues. Check for the event logs at the `Warning` level.
...
The FlexVolume driver is deprecated as of {longhorn-product-name} v0.8.0 and should no longer be used.
...
=== A volume can be attached or detached from the UI, but Kubernetes Pods or StatefulSets cannot use it

==== Issue when using the FlexVolume plug-in
...
Certain vendors choose to change the directory for several reasons. For example, GKE uses `/home/kubernetes/flexvolume` instead.
...
For Debian machines, if you encounter errors similar to the below when installing Linux kernel extra modules, you need to find an available version in the pkg collection websites like https://pkgs.org/search/?q=linux-modules-extra[Packages for Linux and Unix page] rather than directly relying on `uname -r` instead:
...
*Validation*: Run `lspci -t` or check `/sys/kernel/iommu_groups/`. If the NVMe and Bridge are in the same group, you must switch the disk to *AIO mode* in the {longhorn-product-name} UI.
