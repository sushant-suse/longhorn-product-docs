...
____
Note that the volumes created and used through one driver would not be recognized by Kubernetes using the other driver. So please do not switch the driver (for example, during an upgrade) if you have existing volumes created using the old driver.
____
