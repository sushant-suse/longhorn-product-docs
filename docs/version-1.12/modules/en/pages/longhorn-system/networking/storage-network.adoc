...
By Default, {longhorn-product-name} uses the default Kubernetes cluster CNI network that is limited to a single interface and shared with other workloads cluster-wide. In case you have a situation where network segregation is needed, {longhorn-product-name} supports isolating {longhorn-product-name} in-cluster data traffic with the Storage Network setting.
...
{longhorn-product-name} will apply the same annotation to any new instance-manager, backing-image-manager, and backing-image-data-source pods.
...
When all volumes are detached, {longhorn-product-name} attempts to restart all Instance Manager and Backing Image Manager pods to apply the setting.
...
=== Setting Storage Network During {longhorn-product-name} Installation
...
If this is the case, provide a valid `NetworkAttachmentDefinition` and re-run {longhorn-product-name} install.
====

=== Setting Storage Network After {longhorn-product-name} Installation
