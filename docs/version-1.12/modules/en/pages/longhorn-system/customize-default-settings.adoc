...
Modify the default settings in the YAML file as described in xref:_using_helm[Fresh Installation > Using Helm] and then update the settings using
...
[#_upgrade_using_the_longhorn_deployment_yaml_file]
=== Using the Longhorn Deployment YAML File

Modify the config map named `longhorn-default-setting` in the yaml file `longhorn/deploy/longhorn.yaml` as described in xref:_upgrade_using_the_longhorn_deployment_yaml_file[Fresh Installation > Using the Longhorn Deployment YAML File] and then upgrade the Longhorn system using `kubectl`.

=== Using Helm

Modify the default settings in the YAML file as described in xref:_using_helm[Fresh Installation > Using Helm] and then upgrade the Longhorn system using `helm upgrade`.
