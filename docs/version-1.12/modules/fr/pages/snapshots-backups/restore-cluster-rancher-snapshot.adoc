 ...
 ** Un volume reste inchangé{nbsp}: Les utilisateurs n'ont pas besoin de faire quoi que ce soit.
 ** Les données sont mises à jour{nbsp}: Les utilisateurs n'ont généralement pas besoin de faire quoi que ce soit. Longhorn échouera automatiquement les répliques qui ne contiennent pas les dernières données.
 ** Un nouveau volume est créé{nbsp}: Ce volume disparaîtra après la restauration. Les utilisateurs doivent recréer un nouveau volume, lancer xref:data-integrity-recovery/data-recovery/retrieve-volume-data-from-replica.adoc[un volume à réplique unique] basé sur la réplique du volume disparu, puis transférer les données vers le nouveau volume.
 ...
 ** Pour les volumes DR{nbsp}: Les utilisateurs n'ont pas besoin de faire quoi que ce soit. Longhorn refaira une restauration complète.
 ** Certaines opérations sont appliquées à un volume{nbsp}:
  ...
  *** Instantané{nbsp}: Les informations d'instantané du volume doivent être resynchronisées une fois le volume attaché.
...
* Le problème lié sur GitHub est https://github.com/longhorn/longhorn/issues/2228. Dans ce post GitHub, un utilisateur propose une méthode pour restaurer Longhorn sur un nouveau cluster qui ne contient aucune donnée.
