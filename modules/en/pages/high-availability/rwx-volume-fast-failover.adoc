...
To enable the feature, you set xref:references/settings.adoc#rwx-volume-fast-failover-experimental[RWX Volume Fast Failover] to "true".  Existing RWX volumes will need to be restarted to use the feature after the setting is changed.  That is done by scaling the workload down to zero and then back up again.  New volumes will pick up the setting at creation and be configured appropriately.
