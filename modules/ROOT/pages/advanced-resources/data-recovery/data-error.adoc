...
To determine if the error is caused because one of the underlying disks went bad, follow xref:advanced-resources/data-recovery/corrupted-replica.adoc[these steps] to identify corrupted replicas.
...
If all of the methods above failed, use a backup to xref:snapshots-and-backups/backup-and-restore/restore-from-a-backup.adoc[recover the volume.]
