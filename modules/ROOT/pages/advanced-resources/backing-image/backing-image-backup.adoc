...
You must first xref:snapshots-and-backups/backup-and-restore/set-backup-target.adoc[set up a backup target]. If you skip this crucial step, the missing backup target will prevent Longhorn from creating a backup of the backing image.
